...
void linearWeightedLabels(const int* labels, const double* distances, int* result, const int &numValues){
	...
	for (unsigned int i=0; i<numValues; i++)
	{
		...
		//cout << distances[i] << " " << dict[labels[i]] << endl;
	}
	double maxValue = 0;
	int maxKey = -1;
...
void getNToNDistancesIntern(const double *samples, const double *samples2, double *distances, const int &numSamples, const int &numSamples2, const int &numFeatures){
	for (unsigned int i=0; i<numSamples; i++){
...
void getNToNDistances(const double* carrSampleData, const double* carrSampleData2, double* carrDistances, const int& sampleDataRows, const int& sampleData2Rows, const int& numFeatures){
	getNToNDistancesIntern(carrSampleData, carrSampleData2, carrDistances, sampleDataRows, sampleData2Rows, numFeatures);
}
