...
RandomTest::RandomTest(const int& numClasses, const int& numFeatures, const VectorXd &minFeatRange, const VectorXd &maxFeatRange) :
    ...
    //cout << numClasses << " " << numFeatures << " " << m_threshold << endl;
}
...
int OnlineNode::getLeaveCount(){
	if (m_isLeaf)
		return 1;
	else return m_rightChildNode->getLeaveCount() + m_leftChildNode->getLeaveCount();
}

int OnlineNode::getNodeCount(){
	if (m_isLeaf)
		return 1;
	else return 1 + m_rightChildNode->getNodeCount() + m_leftChildNode->getNodeCount();
}
...
int OnlineTree::getComplexity() {
    return m_rootNode->getNodeCount();
}

int OnlineTree::getComplexityNumParamMetric() {
	int leaveCount = m_rootNode->getLeaveCount();
    return m_rootNode->getNodeCount() + (m_rootNode->getNodeCount() - leaveCount) * 2 + leaveCount;
}
...
/*
OnlineRF::OnlineRF(const int& numTrees, const int& numRandomTests, const int& maxDepth, const int& counterThreshold, const int& numClasses, const int& numFeatures, const VectorXd& minFeatRange, const VectorXd& maxFeatRange);
 	m_counter(0.0), m_oobe(0.0) {
 	Hyperparameters hp = Hyperparameters(numTrees, numRandomTests, maxDepth, counterThreshold)
	Classifier(,
    OnlineTree *tree;
    for (int nTree = 0; nTree < hp.numTrees; nTree++) {
        tree = new OnlineTree(hp, numClasses, numFeatures, minFeatRange, maxFeatRange);
...
int OnlineRF::getComplexity() {
	int result = 0;
    for (int nTree = 0; nTree < m_hp->numTrees; nTree++) {
        result += m_trees[nTree]->getComplexity();
    }
    return result;
}

int OnlineRF::getComplexityNumParamMetric() {
	int result = 0;
    for (int nTree = 0; nTree < m_hp->numTrees; nTree++) {
        result += m_trees[nTree]->getComplexityNumParamMetric();
    }
    return result;
}
