...
if (new_model)
   ...
   %ind{MARGIN}
   %(y(ind{MARGIN})*y_new').*kernel(X(:,ind{MARGIN}),X_new,type,scale)
   %disp(['y_new :', num2str(size(y_new))]);
   %disp(['Q_new :', num2str(size(Q_new))]);
